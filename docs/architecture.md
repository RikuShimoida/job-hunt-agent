...
**「自動返信をしない」と「返信下書きの支援」は両立する。** 応募返信メールの**下書き作成**は
claude.ai の Gmail コネクタ経由で行い（送信は本人）、job-hunt-agent（Go）は Gmail へ書き込まない。
Go 側が持つのは下書きの素材（`ApplicationProfile`）だけで、生成・送信のどちらもしない（§7 の ADR）。
...
| `ApplicationProfile` | 応募返信メールの下書き素材（自己紹介・経歴・強み・志望動機）。**採点には使わない**。`Profile` 内に別構造体（`application:`）として持つ |
...
同じ理由から、応募 URL は `SourceURL` とは別の **`ApplyURL`**（`job_postings.apply_url`）に持ち、
...
- **案件として成立しないメール**（案件名・企業名・単価がいずれも取れない事務連絡など）は
  空レコードを作らずスキップする（`model.JobPosting.HasContent`）。スキップ件数はサマリに残す。
  個別項目の欠落（null 保存で継続）とは区別し、「案件そのものが無いメール」だけを弾く
...
`log/slog` の構造化ログに、取得件数・新規件数・重複件数・更新件数・スキップ件数・通知対象件数・
送信成功件数・失敗ソースを出力する。
...
| 2026-07-14 | 応募 URL は `model.JobPosting.ApplyURL`（`job_postings.apply_url`）へ**別カラムで持ち**、`parser.DedupKey` の入力に渡さない | `source_url` へ相乗りさせる（`extractCrowdTech` の当初の意図） / `dedup_key` の優先順を変えて回避する | 応募 URL は**提携企業ごとに共通で、案件ごとに一意ではない**（ROSCA → `share.hsforms.com/…` / TERAZ → `forms.gle/…`）。`DedupKey` は `id:` → `url:` → `hash:` の順であり、**案件 ID の抽出に失敗した回**に共通 URL が `source_url` へ入ると、同一提携企業の複数案件が同じ `url:` 鍵になる。「衝突したら既存行を更新する」仕様により**先に保存した案件が上書きされて消える**（`id:` を `url:` より優先した既存 ADR がまさに避けている事故）。優先順の調整は「案件 ID が取れた回だけ安全」という条件付きの守りにしかならないが、別フィールドにして `DedupKey` の引数から外せば**型で経路を塞げる**。あわせて `MaterialHash` の入力にも含めない（エージェントがフォームを差し替えただけで既通知の全案件が一斉再通知されるため） |
| 2026-07-14 | クラウドテックの応募 URL は `■■■■エントリー方法■■■■` **セクション配下**の最初の `https://` に限って拾う | 本文全体の最初の `https://` を採る / 案件 ID をクエリに含む URL を応募導線とみなす | 実メール末尾の `==============配信停止に関するご案内==============` に `docs.google.com/forms/…&entry.547304507=<案件ID>` があり、**案件 ID がクエリに入るぶん応募導線に見えるが実体は配信停止・問い合わせフォーム**（実 DB の16件すべてで同一のフォーム ID）。これを「応募：」として通知すると利用者が応募したつもりで配信停止フォームを開き、**リンクが無い現状より悪い**。探索をセクション内へ閉じ、次のセクション見出しに入った時点で打ち切ることで取り違えを構造的に防ぐ。なお `extractCrowdTech` にあった `strings.HasPrefix(head, "https://")` は、`splitLabel` が ASCII の `:` も区切りとみなし `https://…` が `key="https"` として直前の `continue` に吸われるため**到達不能**だった（クラウドテック案件の `source_url` が空だった根本原因）。応募 URL を `source_url` へ入れてはならない以上、生かさず削除した |
| 2026-07-14 | 推奨理由の**文言は `matching/scorer.go` 側で自然文にする**。抽出漏れ（単価 / 開始時期 / リモート / 稼働日数）の列挙は**表示層（`notifier/message`）へ集約**し、`scorer` の `RejectionReasons` からは「読み取れなかった」系を削除する | 表示層が `ScoreReasons` の文字列を正規表現などで再解釈して自然文へ整える / 両方が抽出漏れを持ったまま表示層で重複除去する | 通知は「なぜ合うのか・何に注意すべきか」を伝えるのが目的であり、`希望単価以上（800000円）` のような内部表記では読み手に伝わらない。ただし**表示層で文字列を再解釈すると、文言の定義が2箇所へ割れる**（scorer が文言を変えるたびに表示層の正規表現が黙って壊れる）。理由の文言は生成元である scorer が持ち、表示層はそれをそのまま並べる。一方**抽出漏れは scorer と表示層の両方が挙げうるため、どちらか一方へ寄せないと「懸念：」へ二重に出る**。抽出漏れは「希望と合わない理由」ではなく「情報が足りない」という表示上の注意書きであり、判定条件も本文の各フォーマッタが「不明」を出す条件と一致すべきなので表示層へ置く。scorer は「希望と合わない理由」だけを持つ |
| 2026-07-15 | ハイブリッドで**出社日数が nil のとき、scorer は加点も減点もしない**（3分岐）。出社日数不明の懸念は表示層が「出社日数が案件情報に記載されていない」として挙げる | `OnsiteDays == nil` を else に落として「出社頻度が許容範囲を超える」を積む（従来） | 出社日数 nil を「超過が確定した事実」として減点すると、本文（`message.formatRemote` は日数 nil を「不明」表示にし、日数を出さない）と食い違い、§9 の不変条件「本文の『不明』と懸念の内容を食い違わせない」を破る。フォスターネットの `※基本リモート（必要に応じて出社あり）` が hybrid + `OnsiteDays=nil` に正規化されるため実際に起きる。3分岐（nil は無加点 / 許容内は加点 / 超過は減点）にし、不明の注意書きは表示層の抽出漏れ列挙へ寄せる |
...
| 2026-07-17 | 「～85万」のような**上限のみ表記**は `RateMin=nil` / `RateMax=上限` で正規化し、加点も `minimum_rate` 除外もしない。検出は**先頭区切り記号 `〜／～／~` に限定**する | 上限を確定単価とみなし `RateMin=RateMax=上限` に潰す（従来） / 係数（例 0.8）を掛けて評価する / 加点判定を全面的に `RateMin` 基準へ変える | 従来は範囲表記（`75〜85万`）と上限のみ表記（`～85万`）がモデル上まったく同じ（`RateMin`/`RateMax`）へ潰れ、上限額を確定単価として満点評価していた。実態は「上限を目安にスキル見合いで決まる」ため、`target_rate` の20点を誤加点し、`minimum_rate` の除外も上限で評価して誤判定する（PR #19 で実データ経路が開通し顕在化）。係数掛けは推測が混ざるため採らない。加点を全面 `RateMin` 基準にすると範囲表記（`75〜85万`＋`target 80万`）まで加点されなくなり閾値見直しが要るため、**上限のみ表記だけを加点0**にし範囲・単一表記は非退行に保つ（案A-1）。検出をハイフン（`-`・`ー`）まで広げないのは、電話番号・日付のハイフンを上限単価と誤読するため。比較不能の懸念は時給案件と同じく `rejection_reasons` に残す。`MaterialHash` の入力（`materialRate`）が nil を扱うため、運用中の上限のみ案件は一度だけ「更新」再通知される（golden 値は範囲表記の `baseJob` で導くため不変） |

## 8. スコアリング

### 除外条件（1つでも該当したら `rejected`。点数は付けない）

- 月額単価が `minimum_rate` を下回る（**月額表記かつ下限が確定している案件のみ**。時給案件と
  「～85万」のような**上限のみ表記**（`rate_min` が nil）は比較しない。上限が最低希望を下回っても
  下限が読めない以上「下回る」と断定できず、誤除外で良案件を落とすほうが損失が大きい）
...
**「～85万」のような上限のみ表記（`rate_min` が nil / `rate_max` のみ）は希望単価の20点を加点しない。**
上限額を確定単価とみなして加点すると、実態は「上限を目安にスキル見合いで決まる」案件を
過大評価してしまう（実メールで最も多い表記であるため影響が大きい）。加点も除外もせず、
「単価が〜850000円の上限提示で、確定単価が読み取れないため希望単価と比較できない」を
`rejection_reasons`（懸念）に残す（時給案件を比較不能として扱うのと同じ方針。§7 の ADR）。

**ハイブリッド案件で出社日数が読み取れない（`onsite_days` が nil）とき**は、出社頻度の
10点を**加点せず、超過による減点もしない**（`remote_required: false` の運用でも除外しない）。
出社日数を nil のまま「超過が確定した」と減点に倒すと、本文（`message.formatRemote` は
日数 nil を「不明」表示にする）と食い違うため。出社日数が不明であることは通知の「懸念：」に
**出社日数が案件情報に記載されていない**として挙げる（§9）。

加点理由（`score_reasons`）と減点理由（`rejection_reasons`）を両方保持し、通知に表示する。
文言は**利用者が読んで分かる自然文**として `matching/scorer.go` が組み立てる
（`希望単価 800000円 に到達している` など）。表示層は受け取った文字列をそのまま並べるだけで、
正規表現などで再解釈しない（文言の定義を1箇所に保つ。§7 の ADR）。

**`rejection_reasons` は「希望と合わない理由」だけを持つ。** 「単価が読み取れなかった」のような
**抽出漏れは scorer が持たず、表示層（`notifier/message`）が挙げる**（§9）。両方が持つと
通知の「懸念：」へ同じ内容が二重に出るため、責務を片方へ寄せて構造的に防いでいる。
...
| 行 | 出典 | 出力条件 |
|---|---|---|
| `推奨理由：` | `job.ScoreReasons`（`matching` が自然文で組み立てる） | 1件以上あるとき |
| `懸念：` | `job.RejectionReasons` ＋ **抽出漏れ**（`message.missingFields`） | 1件以上あるとき |
| `応募：` | `job.ApplyURL`（クラウドテック） | 非空のとき |
| `URL：` | `job.SourceURL`（フォスターネット） | 非空のとき |

**抽出漏れの列挙は表示層に集約する。** 対象は**単価 / 開始時期 / リモート / 稼働日数の4項目**で、
判定条件は本文の各フォーマッタが「不明」を出す条件と揃える（本文が「不明」なのに懸念へ挙がらない、
あるいはその逆、という食い違いを防ぐ）。加えて、**ハイブリッドで出社日数が読み取れない
（`remote_type=hybrid` かつ `onsite_days` が nil）**とき、勤務行は日数を伴わない「ハイブリッド」を
出すため、「出社日数が案件情報に記載されていない」を懸念へ挙げる（`scorer` も出社日数 nil を
超過扱いにせず加点0にするのと整合。§8）。`matching` 側は抽出漏れを理由に持たない（§8）。

この「本文に『不明』相当が出た項目 ⟺ 懸念に対応する1行が出る」整合は、`message` の
プロパティテスト（`TestConcernsMatchUnknownFields`）が正規化の返しうる状態
（`remote_type × onsite_days × 単価 × 開始時期 × 稼働日数`）を総当たりし、本文を実際に
パースして双方向で担保する。ラベル集合の一致（`TestSnapshotLabelsMatchMaterialFields`）だけでは
「本文が『不明』なのに懸念に挙がらない／その逆」を検知できず、実際に PR #23 の
Medium①（hybrid + 出社日数 nil で本文は日数不明の「ハイブリッド」なのに懸念は超過を断定）が
この穴を通り抜けた。

`応募：` と `URL：` は別物であり、片方へ寄せない。クラウドテックは応募フォーム URL しか持たず、
フォスターネットは案件詳細 URL しか持たないため、統合すると一方のソースで応募導線が消える。
...
| 応募 URL | `■■■■エントリー方法■■■■` **配下の最初の `https://`** → `apply_url` | 持たない |
| 案件詳細 URL | 持たない | `■案件掲載URL：` → `source_url` |
| 重複キー | `■案件ID：JA-086984` → `id:` | 案件掲載 URL → `url:` |

クラウドテックの応募 URL は**エントリー方法セクションの内側だけ**を探す。本文全体から最初の
`https://` を拾うと、末尾の `==============配信停止に関するご案内==============` にある
Google フォーム（`docs.google.com/forms/…&entry.547304507=<案件ID>`）を拾ってしまう。
案件 ID がクエリに入るぶん応募導線に見えるが実体は**配信停止・問い合わせフォーム**であり、
これを「応募：」として通知すると、利用者が応募したつもりで配信停止フォームを開く。
探索範囲をセクション内へ閉じることでこの取り違えを構造的に防ぐ（回帰テストで固定）。

応募 URL は**提携企業ごとに共通で案件ごとに一意ではない**（ROSCA と TERAZ で別の URL になる）。
このため `source_url` へは入れず、`dedup_key` の入力にもしない（§4・§7 の ADR）。
...
launchd はこれらのログへ**追記し続け、ローテーションは行わない**（日次実行のため増加は緩やか
だが、長期運用ではファイルが単調増加する）。ローテーションが必要になった段階で別途検討する。
